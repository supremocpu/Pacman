...
public class PacMan extends JPanel implements ActionListener, KeyListener {
	...
	PacMan () {
		...
		// how long it takes to start timer, milliseconds gone between frames (delay)
	...
	public void move() {
		...
		for (Block wall : walls) {
			...
			if (pacman.x <= 0 && pacman.direction == 'L') {
				pacman.x = pacman.x + pacman.width + boardWidth;
			} 
			
			if (pacman.x >= boardWidth && pacman.direction == 'R') {
				pacman.x = 0;
			}
		}
	...
	public void resetPositions() {
		...
		for (Block ghost : ghosts) {
			ghost.reset();
			char newDirectino = directions[random.nextInt(4)];
			ghost.updateDirection(newDirectino);
		}
	}
